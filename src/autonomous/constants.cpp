...
void default_constants() {
    ...
    chassis.set_control_constants(25, 22, 1.028, 20, 8, 1.015);

    // Each constant set is in the form of (maxVoltage, kP, kI, kD, startI, slew).
	chassis.set_drive_constants(10, 0.9, 0.0, 5.15, 0, 0);
    chassis.set_turn_constants(12, 0.54, 0.01, 4.434, 15, 0);
    chassis.set_heading_constants(6, .370, 0.005, 2.8, 15, 0);
    chassis.set_swing_constants(12, .437, .0295, 3.486, 15, 0);
...
void odom_constants() {
    ...
    chassis.boomerang_drift = 8;  // Change to 8 if you are not running all omni wheels
}
