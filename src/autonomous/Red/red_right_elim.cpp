...
std::string red_right_elim(bool calibrate, mik::auto_variation var, bool get_name) {   
    ...
    chassis.drive_to_point(-48, -48, { .max_voltage = 10 });
    chassis.turn_to_angle(270);
    assembly.tongue.open();
    
    
    chassis.drive_to_point(-63.5, -48, { .max_voltage = 10, .timeout = 1300}); // timeout is time to get both matchloader and blocks
    chassis.drive_to_point(-27, -48, { .wait = false, .max_voltage = 10, .timeout = 2500 });
    wait(400, msec);
    assembly.tongue.close();
    wait(150, msec);
