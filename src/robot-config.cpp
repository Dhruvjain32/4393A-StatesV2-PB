...
Chassis chassis(
    ...
    mik::motor_group({
        mik::motor(PORT8, true, blue_6_1, "left front motor"),
        mik::motor(PORT9, false, blue_6_1, "left middle motor"),
        mik::motor(PORT10, true, blue_6_1, "left back motor"),
    }),
    // Right drivetrain motors
    mik::motor_group({
        mik::motor(PORT20, false, blue_6_1, "right front motor"),
        mik::motor(PORT18, true, blue_6_1, "right middle motor"),
        mik::motor(PORT19, false, blue_6_1, "right back motor"),
    }),
	
    PORT21,  // Inertial sensor port
    358.86,    // Inertial scale (reading after a full 360° turn)
    ...
    PORT17,  // Forward tracker port. PORT0 if unused. Accepts "PORT_A"
    -2,      // Forward tracker wheel diameter (in). Negative flips direction. Pushing robot forward at 0° should increase Y
    -0.25,      // Forward tracker center distance (in). Positive = right of center, negative = left. Can be found automatically with "Get Offsets" on "Config" tab

    PORT16,  // Sideways tracker port. PORT0 if unused. Accepts "PORT_A"
    2,      // Sideways tracker wheel diameter (in). Negative flips direction. Pushing robot right at 0° should increase X
    5.10,     // Sideways tracker center distance (in). Positive = behind center, negative = in front. Can be found automatically with "Get Offsets" on "Config" tab

    // Distance sensors mounted on a face of the robot
    mik::distance_reset({
		mik::distance(PORT5, front_sensor, 3.337, 3.759),
		mik::distance(PORT3, right_sensor, 4.577, -2.303),
		mik::distance(PORT4, left_sensor, -4.598, -2.303)
    })
);
...
mik::motor Assembly::lowerStage(PORT6, true, blue_6_1, "lowerStage");

/* Creating upper intake motor in assembly */
mik::motor Assembly::upperStage(PORT7, true, blue_6_1, "upperStage");

/* Creating pistons in assembly */
mik::piston Assembly::wing(PORT_B);
mik::piston Assembly::locking(PORT_C);
mik::piston Assembly::highGoal(PORT_D);
mik::piston Assembly::tongue(PORT_E);
